...
(() => {
	...
	const urlFull = SW_API + endpoint;

	// Realizar el llamado al URL para obtener a Luke Skywalker

	//? forma en la que lo he hecho segun lei el ejercicio (aunque hace 3 peticiones en vez de 2)
	zip(
		getRequest(urlFull),
		getRequest(urlFull).pipe(
			// Realizar los operadores respectivos aquí
			switchMap((luke) => getRequest(luke["homeworld"]))
		)
	)
		.pipe(
			map(([personaje, lugarOrigen]) => {
				return { personaje, "lugar-de-origen": lugarOrigen };
			})
			// NO TOCAR el subscribe ni modificarlo ==
		)
		.subscribe((res) => {
			console.warn("FORMA MIA SEGUN EL ENUNCIADO DEL EJERCICIO");
			console.log(res);
		}); // ==
	// =======================================

	//$$$ Forma en la que yo lo haria realmente
	getRequest(urlFull)
		.pipe(
		...
		.subscribe((res) => {
			console.warn("FORMA EN LA QUE YO LO HARIA");
			console.log(res);
		});
})();
